...
const BADGE_HORIZONTAL_PADDING = 12; // px (left + right)
const AVG_CHAR_WIDTH = 7; // px
...
// dynamic badge size calculation based on label length
export const getBadgeSize = (label: string) => {
  const textWidth = label.length * AVG_CHAR_WIDTH;
  const width = textWidth + BADGE_HORIZONTAL_PADDING * 2;
